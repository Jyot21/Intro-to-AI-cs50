...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Update cell list if cell in bounds and is not mine or safe
                if 0 <= i < self.height and 0 <= j < self.width:
        ...
        for sentence in self.knowledge:
            if sentence.known_mines():
                lst = copy.deepcopy(sentence.cells)
                for cell in lst:
                    self.mark_mine(cell)
        ...
        #make new inference using knowledge
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                if not sentence1 == sentence2 and sentence1.cells in sentence2.cells:
    ...
    def make_random_move(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                if (i,j) not in self.mines and not (i,j) in self.moves_made:
                    avail.append((i,j))
